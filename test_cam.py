...
parser.add_argument("--config",default='configs/SYSU.yaml',type=str,
                    help="config")
parser.add_argument("--save_dir", default="./results/SYSU", type=str, help="save_dir")
...
parser.add_argument("--model_path", default="/data/zhenghui.zhao/Code/MICloss/DISep_submission/DISep/work_dir_SYSU/checkpoints/2024-06-13-12-02/transwcd_iter_14600.pth", type=str, help="model_path")
...
def test(model, dataset, test_scales=1.0):
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader)):
            ...
            valid_cam, cam_label = cam_to_label(resized_cam, cls_label, cfg=cfg)

            cam_A = imutils.combine_cam_image(imgs=inputs_A.clone(), cam=valid_cam)
            cam_B = imutils.combine_cam_image(imgs=inputs_B.clone(), cam=valid_cam)
            ...
            # 以png格式保存cam结果
            pred_path = args.save_dir + '/SYSU_14600/prediction/' + name[0] + '.png'
            pred_img = Image.fromarray((cam_label.squeeze().cpu().numpy() * 255).astype(np.uint8))
            pred_img.save(pred_path)

            cam_path = args.save_dir + '/SYSU_14600/cams/'

            cam_A_path = os.path.join(cam_path, f"{name[0]}_cam_A.png")
            cam_B_path = os.path.join(cam_path, f"{name[0]}_cam_B.png")



            cam_A.save(cam_A_path)
            cam_B.save(cam_B_path)

            ### FN and FP color ###
            valid_cam = cam_label.squeeze().cpu().numpy()
            ...
            fn_pixels = np.logical_and(valid_cam == 0, labels == 1)  # False Negatives
            label_rgb[fn_pixels] = [0, 0, 255]  # Blue

            # Mark FP pixels as red
            fp_pixels = np.logical_and(valid_cam == 1, labels == 0)  # False Positives
            label_rgb[fp_pixels] = [255, 0, 0]  # Red

            # Save the labeled image
            label_with_fn_fp_path = args.save_dir + '/SYSU_14600/prediction_color/' + name[0] + '.png'
...
if __name__ == "__main__":
    ...
    os.makedirs(args.save_dir + "/SYSU_14600/prediction", exist_ok=True)
    os.makedirs(args.save_dir + "/SYSU_14600/prediction_color", exist_ok=True)
    os.makedirs(args.save_dir + "/SYSU_14600/cams", exist_ok=True)

...
class TransWCD_dual(nn.Module):
    ...
    def forward(self, x1, x2, cam_only=False,):
        ...
        cls_x4 = self.pooling(_c4, (1, 1)) #cls_x4: torch.Size([32, 512, 1, 1])
        cls_x4 = self.classifier(cls_x4)
        cls_x4 = cls_x4.view(-1, self.num_classes-1)
        # print('cls_x4:',cls_x4.size())
        # _c4:_c4: torch.Size([32, 512, 16, 16])
...
class TransWCD_single(nn.Module):
    def __init__(self, backbone, num_classes=None, embedding_dim=256, stride=None, pretrained=None, pooling=None, ):
        ...
        self.dropout = nn.Dropout2d(0.1)
        # self.decoder = MLPHead(feature_strides=self.feature_strides, in_channels=self.in_channels,
        ...
        # Difference Modules
        self.diff = conv_diff_s(in_channels=2 * c4_in_channels, out_channels=c4_in_channels)
    ...
    def forward(self, x1, x2, cam_only=False):
        ...
        # pred_enc: torch.Size([32, 1, 16, 16])
        # _c4: torch.Size([32, 512, 16, 16])
        # cls_x4: torch.Size([32, 1])

        return cls_x4, pred_enc   #, pred_dec

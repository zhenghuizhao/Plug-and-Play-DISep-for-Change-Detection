...
def combine_cam_image(imgs=None, cam=None):
    _imgs = denormalize_img(imgs=imgs)

    cam = F.interpolate(cam, size=_imgs.shape[2:], mode='bilinear', align_corners=False)
    cam_max = cam.max(dim=1)[0]
    cam_heatmap = plt.get_cmap('jet')(cam_max.cpu().numpy())[:, :, :, 0:3] * 255
    cam_cmap = torch.from_numpy(cam_heatmap).permute([0, 3, 1, 2])

    # 将 CAM 图像转换为 uint8 类型的张量
    cam_img_uint8 = (cam_cmap * 0.5 + _imgs.cpu() * 0.5).clamp(0, 255).byte()

    # 将张量转换为 numpy 数组
    cam_img_numpy = cam_img_uint8.numpy()

    # 调整通道顺序并压缩维度
    cam_img_numpy = np.squeeze(np.transpose(cam_img_numpy, (0, 2, 3, 1)))

    # 创建 PIL 图像对象
    cam_img_pil = Image.fromarray(cam_img_numpy)
